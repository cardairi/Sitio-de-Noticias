...
function crearCard(noticias){
    let card = `<div class = 'card'>
                    <a target="_blank" href='${noticias.imgUrl}'><img class="card-imagen" scr='${noticias.imgUrl}' alt="Imagen de la noticia"></a>
                    <p class="card-fecha">'${noticias.fecha}'</p>
                    <h2 class="card-titulo">'${noticias.titulo}'</h2>
                    <p class="card-texto">'${noticias.descripcion}'</p>
                    <p class="card-mas">Ver más</p>
                </div>`;
    app.innerHTML += card;
    console.log(card)
}

function renderizarNacionales() {
...
let seleccionarNacionales = document.querySelector('.noticias-nacionales');
seleccionarNacionales.onClick = renderizarNacionales();

function renderizarInternacionales() {
    for (let i = 1; i < noticias.length; i++) {
        if(noticias[i].tipoNacional != true) {
            crearCard(noticias)
        }
...
let seleccionarInternacionales = document.querySelector('.noticias-internacionales');
seleccionarInternacionales.onClick = renderizarInternacionales();